	...
	mlPushMenu.prototype = {
		...
		_initInteractions: function() {
			...
			var scripts_loaded = function() {
				...
				var trigger_parent = self.trigger.parent();
				var trigger_positon = void 0;

				var fix_height = function() {
					self.$el.css( {
						height: win.height() - self.wrapper.offset().top
					} );
				...
				var reposition_trigger = function() {
					if ( self.trigger.is( ':visible' ) && ! self.open ) {
						...
						if ( void 0 === trigger_positon ) {
							first_time = true;
							trigger_positon = self.trigger.position();
						}

						var trigger_parent_offset = trigger_parent.offset(),
							wrapper_offset = self.wrapper.offset().top;

						self.$el.css( 'top', wrapper_offset );

						self.trigger.css( {
							top: trigger_parent_offset.top + trigger_positon.top - wrapper_offset,
							left: '100%'
						} );
				...
				win.bind( 'resize touchmove', _.debounce( fix_height, 100 ) );
						...
						if( next_level.length ) {
							...
							try {
								...
								if ( target === 'blank' || target === 'new' ) {
									...
									window[target].location = el.attr( 'href' );
								}
							} catch (ex) {
								console.log( e );
							}
