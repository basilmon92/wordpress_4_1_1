...
<div class="logo-wrapper">
	<a href="#" id="mp-menu-trigger" class="icon-b" data-icon="<?php wpv_icon('menu1') ?>"><?php _e('Open/Close Menu', 'honeymoon') ?></a>
	<?php $logo_type = wpv_get_option( 'header-logo-type' ); ?>
	<?php if ( $logo_type === 'image' || $logo_type === 'site-title' ): ?>
		<?php
		...
		?>
		<a href="<?php echo home_url() ?>/" title="<?php bloginfo( 'name' ) ?>" class="logo <?php if ( empty( $logo ) || $logo_type === 'site-title' ) echo 'text-logo' //xss ok ?>" style="min-width:<?php echo $logo_size['width']/2 ?>px"><?php
	...
	<?php else: ?>
		<a href="<?php echo home_url() ?>/" title="<?php bloginfo( 'name' ) ?>">
			<div class="header-names">
				<div class="left-name">
					<div class="top-name"><?php wpvge( 'header-name-left-top' ) ?></div>
					<div class="bottom-name"><?php wpvge( 'header-name-left-bottom' ) ?></div>
				</div>
				<div class="names-separator"><div class="names-separator-inner">&amp;</div></div>
				<div class="right-name">
					<div class="top-name"><?php wpvge( 'header-name-right-top' ) ?></div>
					<div class="bottom-name"><?php wpvge( 'header-name-right-bottom' ) ?></div>
				</div>
			</div>
		</a>
	<?php endif ?>
