	...
	private function markup_handler($id, $attributes=null, $content='') {
		...
		?>
			<div id="<?php echo esc_attr($this->get_uniqid($id)) ?>" class="wpv_ed_column wpv_sortable inner-sortable column column-<?php echo preg_replace('/column-(\d)(\d)/', '$1-$2', $id) ?> expandable" data-basename="column">
				<?php echo $this->getControls('size name add edit clone delete handle', preg_replace('/column-(\d)(\d)/', '$1/$2', $id), __('Column', 'honeymoon'), $attributes) ?>
	...
	private function getControls($controls, $size_str, $name='', $atts = array()) {
		...
		$controls_html = array(
			...
			'size' => "<div class='column-size-wrapper'>
							<div class='column-increase-decrease'><a class='column-increase icon-plus' href='#' title='".__('Increase width', 'honeymoon')."'></a> <a class='column-decrease icon-minus' href='#' title='".__('Decrease width', 'honeymoon')."'></a></div>
							<span class='column-size'>$size_str</span>
						</div>",
			// 'add' => " <a class='column-add icon-plus' href='#' title='".__('Insert element', 'honeymoon')."'></a> ",
			'clone' => " <a class='column-clone icon-copy' href='#' title='".__('Clone', 'honeymoon')."'></a> ",
			'edit' => ' <a class="column-edit icon-edit" href="#" title="'.__('Edit shortcode properties', 'honeymoon').'"></a>',
			'delete' => ' <a class="column-remove icon-remove" href="#" title="'.__('Remove shortcode', 'honeymoon').'"></a>',
			'handle' => '<div class="handlediv" title="'.__('Click to toggle', 'honeymoon').'"><br /></div>',
		);
				...
				foreach($sub_contents as $i=>$text): ?>
					...
					<div>
						<h3 class="title-wrapper clearfix">
							...
							<a class="accordion-remove icon-remove" title="<?php _e('Remove', 'honeymoon') ?>"></a>
							<a class="accordion-clone icon-copy" title="<?php _e('Clone', 'honeymoon') ?>"></a>
							<a class="accordion-background-selector" data-background-image="<?php if ( isset( $params[$i]['background_image'] ) ) echo esc_attr($params[$i]['background_image']) ?>" title="<?php esc_attr_e('Change Pane Background', 'honeymoon') ?>"><?php wpv_icon('image') ?></a>
						</h3>
			...
			case 'tabs':
				...
				?>
					<li>
						...
						<a class="tab-remove icon-remove" title="<?php esc_attr_e('Remove', 'honeymoon') ?>"></a>
						<a class="tab-clone icon-copy" title="<?php esc_attr_e('Clone', 'honeymoon') ?>"></a>
						<a class="wpv-icon-selector-trigger tab-icon-selector vamtam-icon <?php echo wpv_get_icon_type($p['icon']) ?> <?php if(empty($p['icon'])) echo 'no-icon' ?>" data-icon-name="<?php echo $p['icon'] ?>" title="<?php esc_attr_e('Change Icon', 'honeymoon') ?>"><?php wpv_icon($p['icon']) ?></a>
					</li>
				<?php endforeach;
