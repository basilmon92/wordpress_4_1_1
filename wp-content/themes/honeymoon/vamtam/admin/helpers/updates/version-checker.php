	...
	public function check_license() {
		...
		if ( ! is_wp_error( $api_check ) ) {
			if ( $api_check['response']['code'] >= 200 && $api_check['response']['code'] < 300  ) {
				...
				_e( 'Valid Username and API Key', 'honeymoon' );
				echo '</span>'; // xss ok
				...
				if ( ! is_wp_error( $purchase_check ) && $purchase_check['response']['code'] >= 200 && $purchase_check['response']['code'] < 300 && isset( $purchase_result->{'download-purchase'}->download_url ) ) {
					echo '<span style="color: green">'; // xss ok
					_e( 'Valid Purchase Code', 'honeymoon' );
					echo '</span>'; // xss ok
				} else {
					echo '<span style="color: red">'; // xss ok
					_e( 'Invalid Purchase Code', 'honeymoon' );
					echo '</span>'; // xss ok
				}
			} else {
				echo '<span style="color: red">'; // xss ok
				_e( 'Incorrect Username and/or API Key', 'honeymoon' );
				echo '</span>'; // xss ok
			}
		} else {
			echo '<span style="color: red">'; // xss ok
			_e( 'Cannot validate Username/API Key. Please try later; if the problem persists your server might not have the curl PHP extension enabled.', 'honeymoon' );
			echo '</span>'; // xss ok
		}
