...
class wpv_posts extends WP_Widget {
	...
	public function wpv_posts() {
		$widget_options = array(
			'classname' => 'wpv_posts',
			'description' => __("Displays a list of posts/comments", 'honeymoon')
		);
		$this->WP_Widget('wpv_posts', __('Vamtam - Multi widget', 'honeymoon') , $widget_options);
	...
	private function get_section_title($orderby, $single=false) {
		if($orderby == 'comment_count')
			return apply_filters('wpv_multiwidget_tab_title', __('Popular', 'honeymoon'), $orderby, $single);
		if($orderby == 'date')
			return apply_filters('wpv_multiwidget_tab_title', __('Newest', 'honeymoon'), $orderby, $single);
		if($orderby == 'comments')
			return apply_filters('wpv_multiwidget_tab_title', __('Comments', 'honeymoon'), $orderby, $single);
		if($orderby == 'tags')
			return apply_filters('wpv_multiwidget_tab_title', __('Tags', 'honeymoon'), $orderby, $single);
	}
