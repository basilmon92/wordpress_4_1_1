...
class WPV_Flickr_Widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => __( 'Displays photos from Flickr', 'honeymoon' )
		);
		$this->WP_Widget('wpv_flickr', __('Vamtam - Flickr', 'honeymoon'), $widget_ops);
	}

	public function widget( $args, $instance ) {
		extract($args);
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Photos on flickr', 'honeymoon') : $instance['title'], $instance, $this->id_base);
